...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const syncData = useCallback(async () => {
    ...
    if (adminRoute) {
      try {
        ...
        if (response.ok && Array.isArray(result.orders)) o = result.orders;
      } catch (error) { console.warn('Admin order sync failed:', error); }
      b = await dbFetchBookings(currentUser.id);
    } else {
      [o, b] = await Promise.all([dbFetchOrders(currentUser.id, currentUser.role), dbFetchBookings(currentUser.id)]);
    }
  ...
  const addNewProduct = async (data: Omit<Product, 'id' | 'createdAt'>) => { const p: Product = { ...data, id: `prod-${Date.now()}`, createdAt: new Date().toISOString() }; setProducts(prev => [p, ...prev]); if (!(await saveProduct(p))) { setProducts(prev => prev.filter(x => x.id !== p.id)); throw new Error('Product could not be saved'); } return p; };
  ...
  const createOrder=async(paymentMethod:PaymentMethod)=>{
    ...
    const restaurant=restaurants.find(r=>r.id===first.restaurantId)||restaurants[0];
