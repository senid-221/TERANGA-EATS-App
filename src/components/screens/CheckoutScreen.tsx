...
export const CheckoutScreen: React.FC<CheckoutScreenProps> = ({ onBack, onOrderSuccess }) => {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showToast('Veuillez entrer une adresse email valide.'); return; }
    if (!mapsQuery) { showToast('Veuillez renseigner votre localisation de livraison.'); return; }

    setIsProcessing(true);
    setProcessingStep(language === 'fr' ? 'Enregistrement sécurisé de votre commande…' : 'Securely saving your order…');
    try {
      const order = await createOrder(paymentMethod);
      if (order.paymentStatus === 'pending' && paymentMethod !== 'cash_on_delivery') {
        showToast(language === 'fr' ? 'Commande enregistrée. Paiement en attente de confirmation.' : 'Order saved. Payment is awaiting confirmation.');
      } else {
      ...
      showToast(error instanceof Error ? error.message : 'Impossible de créer la commande. Veuillez réessayer.');
    } finally {
  ...
  return (
    <div id="checkout-screen" className="min-h-screen bg-[#FDFBF7] pb-28 max-w-2xl mx-auto p-4 sm:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <button onClick={onBack} disabled={isProcessing} className="w-11 h-11 rounded-2xl bg-white text-[#2D2D2D] flex items-center justify-center shadow-artistic border border-[#F0EDE8] cursor-pointer hover:bg-gray-50 active:scale-95 transition-all"><ArrowLeft className="w-5 h-5" /></button>
        <h2 className="font-heading font-black text-lg sm:text-xl text-[#2D2D2D]">{t('checkoutTitle')}</h2>
        <div className="flex items-center gap-1.5 text-[11px] font-black text-[#006633] bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-200"><ShieldCheck className="w-4 h-4 text-[#006633]" /><span>SSL 256-bit</span></div>
      </div>
      ...
      <div className="pt-2"><Primary3DButton id="btn-place-order-confirm" onClick={handlePlaceOrder} loading={isProcessing} size="lg" icon={<Lock className="w-4 h-4" />}>{isProcessing ? processingStep || 'Traitement...' : `${t('confirmOrder')} • ${cartTotal.toLocaleString()} FCFA`}</Primary3DButton></div>
    </div>
  );
};
