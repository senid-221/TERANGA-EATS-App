...
export const ProductsScreen: React.FC<{ onOpenCart?: () => void }> = ({ onOpenCart }) => {
  ...
  useEffect(() => {
    ...
    const loadCatalog = async () => {
      try {
        const response = await fetch('/api/catalog', { credentials: 'include', cache: 'no-store' });
        const result = await response.json().catch(() => ({}));
        if (!cancelled && response.ok && result.ok) {
          if (Array.isArray(result.products)) setCatalogProducts(result.products);
          if (Array.isArray(result.categories)) setCatalogCategories(result.categories);
        }
      } catch (error) {
        console.warn('Public catalog API failed:', error);
      }
    };
    void loadCatalog();
  ...
  const products = catalogProducts.length ? catalogProducts : contextProducts;
  const categories = catalogCategories.length ? catalogCategories : contextCategories;
  ...
  return <div id="products-screen" className="min-h-screen bg-[#FAF8F5] pb-10">
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-8 pt-5 space-y-5">
      ...
      {visible.length ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{visible.map(p=><ProductCard key={p.id} product={p} onSelect={()=>selectProduct(p)} />)}</div> : <div className="py-16 text-center text-sm text-gray-400">{language==='fr'?'Aucun produit disponible.':'No products available.'}</div>}
    </main>
