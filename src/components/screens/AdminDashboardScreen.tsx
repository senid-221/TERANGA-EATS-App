    ...
    <div id="admin-dashboard" className="p-4 sm:p-6 pb-28 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#006633] via-[#0A522A] to-[#1F2937] text-white rounded-[32px] p-6 sm:p-8 shadow-artistic-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-emerald-700/30">
        <div>
          ...
          <p className="text-xs sm:text-sm text-white/70 mt-1 font-medium">
            Supervision en temps réel des commandes, réservations et base de données Supabase
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            disabled={isSyncing}
            className="px-3.5 py-2 rounded-2xl bg-white/20 hover:bg-white/30 text-white text-xs font-black border border-white/30 flex items-center gap-1.5 shadow-xs cursor-pointer active:scale-95"
          >
          ...
          <span className="px-3.5 py-2 rounded-2xl bg-emerald-500/20 text-emerald-300 text-xs font-black border border-emerald-500/30 flex items-center gap-2 shadow-xs">
            ...
            {isSupabaseConnected ? 'Supabase Connecté' : 'Mode Base de données'}
          </span>
        </div>
      </div>

      {/* Integration Status Badges */}
      <div className="bg-white rounded-[24px] p-4 border border-[#F0EDE8] shadow-xs flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <div className={`w-2.5 h-2.5 rounded-full ${isSupabaseConnected ? 'bg-emerald-500' : 'bg-amber-500'}`} />
            <span className="text-xs font-bold text-gray-700">Supabase DB:</span>
            <span className="text-xs font-black text-[#006633]">
              {isSupabaseConnected ? 'Tables Supabase Actives (PostgreSQL)' : 'Prêt pour configuration .env'}
            </span>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-[28px] p-5 border border-[#F0EDE8] shadow-artistic">
          ...
          <p className="text-[10px] text-[#006633] font-black mt-1">Chiffre d'affaires réel</p>
        </div>
        ...
        <div className="bg-white rounded-[28px] p-5 border border-[#F0EDE8] shadow-artistic">
          ...
          <p className="text-[10px] text-sky-700 font-black mt-1">Dakar & Almadies</p>
        </div>

        <div className="bg-white rounded-[28px] p-5 border border-[#F0EDE8] shadow-artistic">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-gray-500">Réservations Tables</span>
          ...
          <p className="font-heading font-black text-xl text-[#2D2D2D]">
            {bookings.length} réservées
          </p>
          <p className="text-[10px] text-purple-700 font-black mt-1">En direct</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[32px] p-6 border border-[#F0EDE8] shadow-artistic space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#F0EDE8]">
          <div>
            ...
            <p className="text-xs text-gray-500 font-medium">Gérez et mettez à jour les statuts en direct dans la base de données</p>
          </div>
        ...
        <div className="space-y-3">
          {orders.filter((o) => (selectedStatusFilter === 'all' ? true : o.orderStatus === selectedStatusFilter))
            .length === 0 ? (
            <div className="text-center py-10 text-gray-400">
              <ShoppingBag className="w-8 h-8 mx-auto mb-2 opacity-40" />
              <p className="text-xs font-bold">Aucune commande enregistrée pour ce filtre.</p>
              <p className="text-[11px]">Passez une commande en mode client pour la voir apparaître ici en direct.</p>
            </div>
          ) : (
            orders
              .filter((o) => (selectedStatusFilter === 'all' ? true : o.orderStatus === selectedStatusFilter))
              .map((order) => (
                <div
                  key={order.id}
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[11px] text-gray-500 flex items-center gap-1 font-medium">
                      ...
                      {order.deliveryAddress?.neighborhood || 'Dakar'} ({order.deliveryAddress?.streetAddress || ''})
                    </p>
                  </div>
                    ...
                    {(['preparing', 'ready', 'delivering', 'delivered'] as OrderStatus[]).map((st) => (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          showToast(`Statut commande ${order.id} mis à jour : ${st}`);
                        }}
