...
const AdminDashboardBridge: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  const { syncData, showToast } = useApp();
    ...
    const handleClick = async (event: MouseEvent) => {
      ...
      if (label === 'Actualiser DB') {
        ...
        try {
          ...
          showToast('Impossible d’actualiser la base de données.');
        }
        return;
      }
      ...
      const localBadge = Array.from(document.querySelectorAll('span')).find(
        (node) => node.textContent?.trim() === '● Base locale'
      );
      if (localBadge && localBadge.contains(target)) {
        event.preventDefault();
        event.stopPropagation();
        showToast('Mode Base locale actif. Les données locales restent disponibles.');
      }
    };
