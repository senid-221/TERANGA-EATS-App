  ...
  useEffect(() => {
    const handleDashboardClick = (event: MouseEvent) => {
      ...
      if (label === 'Déconnexion') {
        ...
        void fetch('/api/admin/logout', { method: 'POST', credentials: 'include' }).finally(() => {
          window.location.replace('/admin');
        });
        return;
      }
      const connectedBadge = Array.from(document.querySelectorAll('span')).find(node => node.textContent?.trim() === '● Supabase Connecté');
      if (connectedBadge?.contains(target) && SUPABASE_PROJECT_URL) {
        event.preventDefault();
        event.stopPropagation();
    ...
    return () => document.removeEventListener('click', handleDashboardClick, true);
  }, []);
