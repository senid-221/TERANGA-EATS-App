...
const MapController: React.FC<{ center: { lat: number; lng: number }; zoom: number }> = ({ center, zoom }) => { const map = useMap(); useEffect(() => { if (map) { map.panTo(center); map.setZoom(zoom); } }, [map, center.lat, center.lng, zoom]); return null; };
...
export const OrderDeliveryMap: React.FC<OrderDeliveryMapProps> = ({ restaurant, deliveryAddress, driver, estimatedTime = '25–35 min' }) => {
  ...
  return <div className="relative w-full h-72 sm:h-80 rounded-[32px] overflow-hidden shadow-artistic-lg border border-[#F0EDE8] bg-slate-900">
    <GoogleMapsWrapper fallbackHeight="h-72 sm:h-80">
      <Map defaultCenter={mapCenter} defaultZoom={14} mapId="DEMO_MAP_ID" internalUsageAttributionIds={['gmp_mcp_codeassist_v1_aistudio']} gestureHandling="greedy" disableDefaultUI={false} className="w-full h-full">
        ...
        <AdvancedMarker position={{ lat: restLat, lng: restLng }} onClick={() => setActiveMarker('restaurant')} title={restaurant.name}><div className="flex flex-col items-center cursor-pointer"><div className="w-10 h-10 rounded-2xl bg-[#FFCC00] text-amber-950 flex items-center justify-center shadow-xl ring-4 ring-[#FFCC00]/40 border border-amber-400"><Utensils className="w-5 h-5" /></div><span className="text-[10px] font-black text-white bg-black/80 px-2 py-0.5 rounded-md mt-1 whitespace-nowrap">{restaurant.name}</span></div></AdvancedMarker>
        {driverPosition && driver && <AdvancedMarker position={driverPosition} onClick={() => setActiveMarker('driver')} title={driver.name}><div className="flex flex-col items-center cursor-pointer"><div className="relative"><div className="w-12 h-12 rounded-full bg-[#006633] text-[#FFCC00] flex items-center justify-center shadow-2xl ring-4 ring-white border-2 border-[#FFCC00]"><Motorbike className="w-6 h-6" /></div><span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-[#E8702A] animate-ping" /></div><span className="text-[10px] font-black text-white bg-[#006633] px-2.5 py-0.5 rounded-full mt-1 shadow-md whitespace-nowrap">{driver.name} 🛵</span></div></AdvancedMarker>}
        <AdvancedMarker position={{ lat: destLat, lng: destLng }} onClick={() => setActiveMarker('destination')} title={deliveryAddress.neighborhood}><div className="flex flex-col items-center cursor-pointer"><div className="w-10 h-10 rounded-full bg-red-600 text-white flex items-center justify-center shadow-xl ring-4 ring-red-400/40"><MapPin className="w-5 h-5 fill-white text-red-600" /></div><span className="text-[10px] font-black text-white bg-black/80 px-2 py-0.5 rounded-md mt-1 whitespace-nowrap">{deliveryAddress.neighborhood}</span></div></AdvancedMarker>
