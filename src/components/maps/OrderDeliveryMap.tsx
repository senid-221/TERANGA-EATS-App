...
export const OrderDeliveryMap: React.FC<OrderDeliveryMapProps> = ({ restaurant, deliveryAddress, driver, estimatedTime = '25–35 min', progressPercent = 0 }) => {
  ...
  const fallbackLat = restLat + (destLat - restLat) * (Math.max(0, Math.min(100, progressPercent)) / 100);
  const fallbackLng = restLng + (destLng - restLng) * (Math.max(0, Math.min(100, progressPercent)) / 100);
  const driverLat = hasGps ? Number(driver!.currentLat) : fallbackLat;
  const driverLng = hasGps ? Number(driver!.currentLng) : fallbackLng;
  ...
  const mapCenter = focusTarget === 'driver' ? { lat: driverLat, lng: driverLng } : { lat: (restLat + destLat) / 2, lng: (restLng + destLng) / 2 };
    ...
    <div className="relative w-full h-72 sm:h-80 rounded-[32px] overflow-hidden shadow-artistic-lg border border-[#F0EDE8] bg-slate-900">
      <GoogleMapsWrapper fallbackHeight="h-72 sm:h-80">
        <Map defaultCenter={mapCenter} defaultZoom={14} mapId="DEMO_MAP_ID" internalUsageAttributionIds={['gmp_mcp_codeassist_v1_aistudio']} gestureHandling="greedy" disableDefaultUI={false} className="w-full h-full">
          ...
          {driver && (
            <AdvancedMarker position={{ lat: driverLat, lng: driverLng }} onClick={() => setActiveMarker('driver')} title={driver.name}>
              <div className="flex flex-col items-center cursor-pointer"><div className="relative"><div className="w-12 h-12 rounded-full bg-[#006633] text-[#FFCC00] flex items-center justify-center shadow-2xl ring-4 ring-white border-2 border-[#FFCC00]"><Motorbike className="w-6 h-6" /></div>{hasGps && <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-[#E8702A] animate-ping" />}</div><span className="text-[10px] font-black text-white bg-[#006633] px-2.5 py-0.5 rounded-full mt-1 shadow-md whitespace-nowrap">{driver.name} 🛵</span></div>
            </AdvancedMarker>
          )}
          ...
          {activeMarker === 'driver' && driver && <InfoWindow position={{ lat: driverLat, lng: driverLng }} onCloseClick={() => setActiveMarker(null)}><div className="p-1 text-slate-900 max-w-[210px]"><p className="font-black text-xs text-[#006633]">🛵 {driver.name}</p><p className="text-[11px] text-gray-600 mt-1">{driver.vehicleType} • {driver.vehiclePlate}</p><p className="text-[10px] text-emerald-700 font-bold mt-1">{hasGps ? 'Position GPS du livreur' : 'Position GPS en attente'}</p></div></InfoWindow>}
          {activeMarker === 'restaurant' && <InfoWindow position={{ lat: restLat, lng: restLng }} onCloseClick={() => setActiveMarker(null)}><div className="p-1 text-slate-900"><p className="font-black text-xs text-amber-700">{restaurant.name}</p><p className="text-[11px] text-gray-600 mt-1">{restaurant.address}</p></div></InfoWindow>}
          {activeMarker === 'destination' && <InfoWindow position={{ lat: destLat, lng: destLng }} onCloseClick={() => setActiveMarker(null)}><div className="p-1 text-slate-900"><p className="font-black text-xs text-red-700">Adresse de livraison</p><p className="text-[11px] text-gray-600 mt-1">{deliveryAddress.neighborhood}{deliveryAddress.street ? ` • ${deliveryAddress.street}` : ''}</p></div></InfoWindow>}
        </Map>
      </GoogleMapsWrapper>

      <div className="absolute top-3 left-3 bg-black/75 backdrop-blur-md rounded-2xl p-2 px-3 text-white text-xs flex items-center gap-2 border border-white/10 z-10"><span className={`w-2.5 h-2.5 rounded-full ${hasGps ? 'bg-emerald-400 animate-ping' : 'bg-gray-400'}`} /><span className="font-bold text-[11px] tracking-wide">{hasGps ? 'GPS livreur en direct' : 'Localisation livraison'}</span></div>
      ...
      <div className="absolute bottom-3 right-3 z-10"><button onClick={() => setFocusTarget(focusTarget === 'driver' ? 'all' : 'driver')} className="px-3 py-1.5 rounded-xl bg-white/90 hover:bg-white text-slate-900 text-xs font-bold shadow-lg backdrop-blur-xs flex items-center gap-1.5 cursor-pointer border border-gray-200 active:scale-95"><Navigation className="w-3.5 h-3.5 text-[#006633]" /><span>{focusTarget === 'driver' ? 'Vue globale' : 'Suivre livreur'}</span></button></div>
    </div>
  );
};