...
create index if not exists order_notifications_status_idx on public.order_notifications(status, updated_at);
...
create trigger order_notifications_set_updated_at before update on public.order_notifications for each row execute function public.set_updated_at();
...
grant usage, select on sequence public.order_notifications_id_seq to service_role;
