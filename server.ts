...
const buildOrderMessage = (order: OrderPayload) => {
  ...
  return [
    ...
    `🆔 Commande : *${order.id}*`, `👤 Client : *${order.customerName}*`, `📱 WhatsApp/Tél : *${order.customerPhone}*`,
    `🍽️ Restaurant : *${order.restaurantName}*`, '', '🛒 *Produits :*', items || '• Aucun produit', '',
    `Sous-total : ${formatMoney(order.subtotal)}`, `Livraison : ${formatMoney(order.deliveryFee)}`, `Réduction : ${formatMoney(order.discount)}`,
    `💰 *TOTAL : ${formatMoney(order.total)}*`, `💳 Paiement : ${order.paymentMethod}`, '',
    `📍 *Livraison :* ${addressText}`, address.instructions ? `📝 Instructions : ${address.instructions}` : '',
...
const sendWhatsAppOrderNotification = async (order: OrderPayload) => {
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      messaging_product: 'whatsapp', recipient_type: 'individual', to: adminNumber.replace(/\D/g, ''), type: 'text',
      text: { preview_url: true, body: buildOrderMessage(order) },
    }),
  });
...
const clerk = process.env.CLERK_SECRET_KEY && process.env.CLERK_PUBLISHABLE_KEY
  ...
  : null;
const adminDb: SupabaseClient | null = supabaseUrl && supabaseServiceRoleKey
...
const productRow = (p: any) => ({
  ...
  prep_time_minutes: p.prepTimeMinutes ?? 20,
...
async function startServer() {
  ...
  // Secure Admin Product CRUD: Clerk verifies the session, then the server uses Supabase service-role.
  app.post('/api/admin/products', async (req, res) => {
    ...
    if (!product?.id || !product?.restaurantId || !product?.categoryId || !product?.nameFR || !product?.nameEN) {
      return res.status(400).json({ ok: false, error: 'Invalid product payload.' });
    }
    const { error } = await adminDb.from('products').insert(productRow(product));
  ...
  app.put('/api/admin/products/:id', async (req, res) => {
    if (!(await authenticateAdmin(req, res))) return;
    if (!adminDb) return res.status(503).json({ ok: false, error: 'Supabase server credentials are not configured.' });
    const product = { ...(req.body?.product || {}), id: req.params.id };
    const { id, ...updates } = productRow(product);
    const { error } = await adminDb.from('products').update(updates).eq('id', id);
    if (error) return res.status(400).json({ ok: false, error: error.message });
    return res.json({ ok: true });
  });
  ...
  app.patch('/api/admin/orders/:id/status', async (req, res) => {
    ...
    if (!status) return res.status(400).json({ ok: false, error: 'Order status is required.' });
    ...
    const { error } = await adminDb.from('orders').update({
      order_status: status,
