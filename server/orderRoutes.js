...
const DELIVERY_ZONES = new Map([
  ['Dakar Plateau', { fee: 500, time: '20–30 min' }],
  ['Les Almadies', { fee: 700, time: '25–35 min' }],
  ['Ngor & Île de Ngor', { fee: 700, time: '25–40 min' }],
  ['Ouakam & Monument de la Renaissance', { fee: 500, time: '20–35 min' }],
  ['Mermoz / Sacré-Cœur', { fee: 500, time: '20–30 min' }],
  ['Fann Résidence / Point E', { fee: 500, time: '15–25 min' }],
  ['Yoff & Tonghor', { fee: 600, time: '25–35 min' }],
  ['Sicap Liberté (1 à 6)', { fee: 500, time: '20–30 min' }],
  ['Parcelles Assainies', { fee: 800, time: '30–45 min' }],
  ['Hann Maristes', { fee: 700, time: '25–40 min' }],
]);
...
const canonicalizeOptions = (product, selected) => {
  ...
  for (const option of selections) {
    ...
    const count = (selectedByGroup.get(group.id) || 0) + 1;
    if (count > Math.max(1, Number(group.maxSelections) || 1)) return { valid: false, options: [], price: 0 };
    selectedByGroup.set(group.id, count);
    canonical.push({
      groupId: group.id,
      groupName: cleanText(group.nameFR || group.nameEN, 120),
      choiceId: choice.id,
      choiceName: cleanText(choice.nameFR || choice.nameEN, 160),
      price: Math.max(0, Math.round(Number(choice.price) || 0)),
    });
  }

  for (const group of groups) {
    if (group?.required && !selectedByGroup.has(group.id)) return { valid: false, options: [], price: 0 };
  }
...
const productSnapshot = (product) => ({
  ...
  originalPrice: product.original_price == null ? undefined : Math.round(Number(product.original_price) || 0),
  available: product.available !== false,
  rating: Number(product.rating) || 0,
  reviewCount: Number(product.review_count) || 0,
  prepTimeMinutes: Number(product.prep_time_minutes) || 20,
  isSpicy: Boolean(product.is_spicy),
  isPopular: Boolean(product.is_popular),
  isSignature: Boolean(product.is_signature),
  ingredientsFR: Array.isArray(product.ingredients_fr) ? product.ingredients_fr : [],
  ingredientsEN: Array.isArray(product.ingredients_en) ? product.ingredients_en : [],
...
export const registerOrderRoutes = (app, { supabase, notifyWhatsApp }) => {
  app.post('/api/orders', async (req, res) => {
    ...
    try {
      ...
      const clientItems = Array.isArray(payload.items) ? payload.items.slice(0, 50) : [];
      const paymentMethod = cleanText(payload.paymentMethod, 40);
      const idempotencyKey = cleanText(req.headers['x-idempotency-key'] || payload.idempotencyKey, 160);

      if (!name || !validPhone(phone) || !validEmail(email) || !restaurantId || !clientItems.length || !PAYMENT_METHODS.has(paymentMethod)) {
        return res.status(400).json({ ok: false, error: 'Complete and valid customer, restaurant, items and payment information are required.' });
      }
      ...
      if (existingError) {
        ...
        return res.status(500).json({ ok: false, error: 'Order system is not fully migrated. Run the production hardening migration.' });
      }
      if (existingOrder) return res.status(200).json({ ok: true, duplicate: true, notificationSent: true, order: publicOrder(existingOrder) });
      ...
      const zone = DELIVERY_ZONES.get(address.neighborhood);
      if (!zone) return res.status(400).json({ ok: false, error: 'This delivery neighborhood is not supported.' });
      ...
      for (const item of clientItems) {
        ...
        const optionResult = canonicalizeOptions(product, item?.selectedOptions);
        if (!optionResult.valid) return res.status(400).json({ ok: false, error: 'One or more product options are invalid or incomplete.' });
        const snapshot = productSnapshot(product);
        const unitPrice = snapshot.price + optionResult.price;
      ...
      const row = {
        ...
        created_at: now,
        estimated_delivery_time: zone.time || restaurant.estimated_delivery_time || '25–35 min',
      };
      ...
      if (insertError || !saved) {
        ...
        if (insertError?.code === '23505') {
          const { data: concurrent } = await supabase.from('orders').select('*').eq('idempotency_key', idempotencyKey).maybeSingle();
          if (concurrent) return res.status(200).json({ ok: true, duplicate: true, notificationSent: true, order: publicOrder(concurrent) });
        }
