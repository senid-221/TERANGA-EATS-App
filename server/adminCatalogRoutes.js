const RESOURCE_CONFIG = {
  restaurants: {
    ...
    select: '*',
    normalize: (x) => ({
      id: String(x.id || ''), name: String(x.name || ''), description_fr: String(x.description_fr || ''), description_en: String(x.description_en || ''),
      cuisine: String(x.cuisine || ''), cuisine_types: Array.isArray(x.cuisine_types) ? x.cuisine_types : [], tags: Array.isArray(x.tags) ? x.tags : [],
      rating: Number(x.rating || 0), review_count: Number(x.review_count || 0), delivery_time: String(x.delivery_time || ''), estimated_delivery_time: String(x.estimated_delivery_time || ''),
      delivery_fee: Number(x.delivery_fee || 0), min_order: Number(x.min_order || 0), address: String(x.address || ''), neighborhood: String(x.neighborhood || ''),
      latitude: x.latitude == null ? null : Number(x.latitude), longitude: x.longitude == null ? null : Number(x.longitude), phone: String(x.phone || ''),
      cover_image_url: x.cover_image_url || null, logo_url: x.logo_url || null, is_halal: x.is_halal !== false, is_popular: Boolean(x.is_popular), is_promoted: Boolean(x.is_promoted),
      is_partner: x.is_partner !== false, is_open: x.is_open !== false, is_featured: Boolean(x.is_featured)
    }),
...
const fromClient = (resource, item) => {
  ...
  const map = {
    name: source.name,
    description_fr: source.descriptionFR, description_en: source.descriptionEN,
    cuisine: source.cuisine, cuisine_types: source.cuisineTypes, tags: source.tags,
    delivery_time: source.deliveryTime, estimated_delivery_time: source.estimatedDeliveryTime,
    delivery_fee: source.deliveryFee, min_order: source.minOrder, address: source.address, neighborhood: source.neighborhood,
    latitude: source.latitude, longitude: source.longitude, phone: source.phone, cover_image_url: source.coverImageUrl, logo_url: source.logoUrl,
    is_halal: source.isHalal, is_popular: source.isPopular, is_promoted: source.isPromoted, is_partner: source.isPartner,
    is_open: source.isOpen, is_featured: source.isFeatured,
    name_fr: source.nameFR, name_en: source.nameEN, image_url: source.imageUrl, icon_name: source.iconName, sort_order: source.sortOrder, dish_count: source.dishCount,
    code: source.code, title_fr: source.titleFR, title_en: source.titleEN, description_fr: source.descriptionFR, description_en: source.descriptionEN,
    discount_type: source.discountType, discount_value: source.discountValue, min_order_value: source.minOrderValue,
    valid_until: source.endDate || source.validUntil, is_active: source.active
  };
  const out = {};
  ...
  app.put('/api/admin/settings', async (req, res) => {
    ...
    try {
      for (const key of allowed) {
        ...
        const value = { value: String(settings[key] ?? '') };
      ...
      res.status(400).json({ ok: false, error: 'Unable to save application settings.' });
    }
  });
}
