...
const notifyWhatsApp = async (order) => {
  ...
  const body = [
    ...
    order.customer_email ? `✉️ Email : ${order.customer_email}` : '',
    '', '🛒 *Produits :*', items || '• Aucun produit', '',
...
// Public order notification: the order must already exist in Supabase.
// It is intentionally limited to notification only; it cannot read or modify arbitrary data.
